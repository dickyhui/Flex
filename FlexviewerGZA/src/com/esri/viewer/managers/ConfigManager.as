...
public class ConfigManager extends EventDispatcher
{
    ...
    private function configService_resultHandler(event:ResultEvent):void
    {
        try
        {
            ...
            var esriLogoVisible:Boolean = configXML.map.@esrilogovisible[0] ? configXML.map.@esrilogovisible == "true" : false;
            ...
            if (arcGISWebMapItemID)
            {
                ...
                {
                    //trace(ObjectUtil.toString(result.item));
                    //trace(ObjectUtil.toString(result.itemData, null, [ "MapItems", "Slides", "widgets" ]));
                    if (!title.value)
                    {
                    ...
                    for (i = 0; i < layers.length; i++)
                    {
                        ...
                        if (lyrXML)
                        {
                            if (isOpLayer)
                            {
                                configData.opLayers.push(getLayerObject(lyrXML, i, true, bingKey, webmapLayer));
                            }
                            else
                            {
                                ...
                                configData.basemaps.push(getLayerObject(lyrXML, i, false, bingKey, webmapLayer));
                            }
                        }
                    }
                    if (!initialExtent)
            ...
            function createWidgets(widgetList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null):void
            {
                for (var p:int = 0; p < widgetList.length(); p++)
                {
					...
					var wMenu:String = widgetList[p].@menu;
					var wNeedLogin:Boolean = widgetList[p].@needLogin == "true" ? true:false;
            ...
            for (i = 0; i < wContainers.length(); i++)
            {
                ...
                function getWidgetList(wgList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null):void
                {
                    for (j = 0; j < wgList.length(); j++)
                    {
						...
						var wNeedLogin:Boolean = wgList[j].@needLogin == "true" ? true:false;
        ...
        {
            ViewerContainer.showError(StringUtil.substitute(getDefaultString("parseConfigErrorText"), error.message));
        }
    }

    private static function getLayerObject(obj:XML, num:Number, isOpLayer:Boolean, bingkey:String, webmapLayer:Layer = null):Object
    {
        ...
        if (type == "tiled" || type == "dynamic")
        {
            ...
            {
                subLayers.push({ id: String(subLayersList[i].@id), info: subLayersList[i].@info, infoConfig: subLayersList[i].@infoconfig, popUpConfig: subLayersList[i].@popupconfig });
            }
        }
        ...
        resultObject.layer = webmapLayer;
        if (!isOpLayer)
        {
