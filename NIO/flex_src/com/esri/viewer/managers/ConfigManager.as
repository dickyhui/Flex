...
public class ConfigManager extends EventDispatcher
{
    ...
    private function configService_resultHandler(event:ResultEvent):void
    {
        try
        {
			...
			var headerContainer:XMLList = configXML.headerContainer;
			for (i = 0; i < headerContainer.children().length(); i++)
			{
				var xmlObject:XML = headerContainer.children()[i];
				if (xmlObject.name() == "headerGroup")
				{
					var headerGroupList:XMLList = XMLList(xmlObject);
					createHeaders(headerGroupList.header, true, headerGroupList.header.length(), headerGroupList.@label, headerGroupList.@icon);
				}
				else
				{
					var headerList:XMLList = XMLList(xmlObject);
					createHeaders(headerList, false);
				}
			}
			
			function createHeaders(headerList:XMLList, grouped:Boolean, groupLength:Number = 0, groupLabel:String = null, groupIcon:String = null):void
			{
				var headerListLength:int = headerList.length();
				for (var p:int = 0; p < headerListLength; p++)
				{
					// if grouped
					...
					var wLabel:String = headerList[p].@label;
					var wIcon:String = headerList[p].@icon;
					var wConfig:String = headerList[p].@config + GlobalProperty.version;
					var wPreload:String = headerList[p].@preload;
					var wWidth:String = headerList[p].@width;
					var wHeight:String = headerList[p].@height;
					var wUrl:String = headerList[p].@url;
					var wx:String = headerList[p].@x;
					var wy:String = headerList[p].@y;
					var wLeft:String = headerList[p].@left;
					var wTop:String = headerList[p].@top;
					var wRight:String = headerList[p].@right;
					var wBottom:String = headerList[p].@bottom;
